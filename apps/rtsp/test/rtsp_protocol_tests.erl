...
padding_test() ->
  {ok, P} = rtsp_protocol:init([[{consumer,self()}]]),
  ...
  {reply, _, P2} = lists:foldl(fun(#stream_info{track_id = N} = S,{reply,_,P_}) ->
    rtsp_protocol:handle_call({add_channel, N - 1, S, tcp}, from, P_)
  end, {reply, ok, P}, Streams),

  {ok, RTP} = file:read_file("../test/padding.rtp"),
  {ok, _, []} = rtsp_protocol:decode_rtp(RTP, element(14,P2)),
  ok.
