...
-define(assertBinEqual(Expect, Expr),
  ((fun (__X) ->
      case (Expr) of
    __X -> ok;
...
skip_common_prefix(<<C,A/binary>>, <<C,B/binary>>) ->
  skip_common_prefix(A, B);
skip_common_prefix(A, B) -> {A,B}.
...
sanyo_hd2300(_Config) ->
  #media_info{streams = [
    #stream_info{content = video, codec = h264, params = 
      #video_params{width = 640, height = 360, nals = [<<103,66,240,30,145,176,40,11,254,92,
      5,184,40,8,10,0,0,7,210,0,1,134,161,8,0,0,0,0,0>>,<<104,206,56,128>>]}},
    #stream_info{content = data}
  ]} = sdp:decode(sanyo_hd2300_sdp()).
...
encoder(_Config) ->
  ...
  Media = #media_info{streams = [h264_stream_info(), aac_stream_info()], options = [{sdp_session, Session}]},
  ?assertBinEqual(full_server_sdp(), sdp:encode(Media)).
...
ess_sdp() ->
<<"v=0
o=- 2623444107 2623444107 IN IP4 0.0.0.0
s=ch1-s1
c=IN IP4 0.0.0.0
a=DevVer:pusher
a=GroupName:IPCAM
...
a=fmtp:96 profile-level-id=42E02A; sprop-parameter-sets=Z0LgKtoBkAlx,aM4wpIA=; packetization-mode=1
b=AS:4000
a=framesize:96 1600-1200
