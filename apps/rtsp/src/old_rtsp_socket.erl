...
read_raw(URL, Options) ->
  ...
  ConnectResult = connect(RTSP, URL, Options),
  ok == ConnectResult orelse erlang:throw(ConnectResult),
  {ok, _Methods} = options(RTSP, Options),
  {ok, MediaInfo, AvailableTracks} = describe(RTSP, Options),
  Tracks = 
  ...
  [ok = setup(RTSP, Track, Options) || Track <- Tracks],
  ok = play(RTSP, Options),
  {ok,RTSP,MediaInfo}.
...
handle_call({connect, _, Options} = Call, From, #rtsp_socket{} = RTSP) ->
  old_rtsp_inbound:handle_call(Call, From, RTSP#rtsp_socket{dump_traffic = proplists:get_value(dump_traffic, Options, true)});

handle_call({consume, _Consumer} = Call, From, RTSP) ->
  old_rtsp_inbound:handle_call(Call, From, RTSP);
...
handle_info(#video_frame{} = Frame, #rtsp_socket{timeout = Timeout} = Socket) ->
  {noreply, old_rtsp_outbound:encode_frame(Frame, Socket), Timeout};
...
handle_response(#rtsp_socket{state = Request, auth_type = basic, auth_info = AuthInfo, pending = From} = Socket, {response, 401, _Message, Headers, _Body}) ->
  case proplists:get_value('Www-Authenticate', Headers) of
    [digest|Digest] ->
      ...
      {noreply, Socket1, _T} = old_rtsp_inbound:handle_call({request,Request}, From, Socket#rtsp_socket{auth_type = digest, auth = DigestAuth}),
      Socket1;
    _ ->
...
handle_response(#rtsp_socket{state = play} = Socket, {response, 200, _Message, Headers, _Body}) ->
  Socket1 = old_rtsp_inbound:sync_rtp(Socket, Headers),
  reply_pending(Socket1#rtsp_socket{state = undefined});
...
handle_request({request, 'DESCRIBE', URL, Headers, Body}, Socket) ->
  old_rtsp_outbound:handle_describe_request(Socket, URL, Headers, Body);
...
handle_request({request, 'PLAY', URL, Headers, Body}, #rtsp_socket{} = Socket) ->
  old_rtsp_outbound:handle_play_request(Socket, URL, Headers, Body);
...
handle_request({request, 'ANNOUNCE', URL, Headers, Body}, Socket) ->
  old_rtsp_inbound:handle_announce_request(Socket, URL, Headers, Body);

handle_request({request, 'PAUSE', _URL, Headers, _Body}, #rtsp_socket{direction = in} = State) ->
  old_rtsp_inbound:handle_pause(State, _URL, Headers, _Body);

handle_request({request, 'PAUSE', _URL, Headers, _Body}, #rtsp_socket{direction = out} = State) ->
  old_rtsp_outbound:handle_pause_request(State, _URL, Headers, _Body);
%
...
handle_request({request, 'SETUP', URL, Headers, Body}, #rtsp_socket{} = Socket) ->
  ...
  case proplists:get_value(mode, Transport) of
    'receive' -> old_rtsp_inbound:handle_receive_setup(Socket, URL, Headers, Body);
    _ -> old_rtsp_outbound:handle_play_setup(Socket, URL, Headers, Body)
  end;
...
send_teardown(#rtsp_socket{socket = Socket, url = URL, auth = Auth, seq = Seq} = RTSP) ->
  ...
  old_rtsp_inbound:dump_io(RTSP, Call),
