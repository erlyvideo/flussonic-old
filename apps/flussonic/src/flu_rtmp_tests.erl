...
h264_config() ->
  ...
  #video_frame{content = video, codec = h264, flavor = config, dts = 0, pts = 0, body = Body}.

aac_config() ->
  #video_frame{content = audio, codec = aac, flavor = config, dts = 0, pts = 0, body = <<17,144>>, sound = ?S}.
...
publish(h264_aac, #env{rtmp= RTMP, stream=Stream}) ->
  ...
  rtmp_publish:send_frame(RTMP, Stream, aac_config()),
  
  rtmp_publish:send_frame(RTMP, Stream, #video_frame{content = video, codec = h264, flavor = keyframe, dts = 0, pts = 0, body = <<"keyframe">>}),
  rtmp_publish:send_frame(RTMP, Stream, #video_frame{content = audio, codec = aac, flavor = frame, dts = 0, pts = 0, body = <<"aac">>, sound = ?S}),
  rtmp_publish:send_frame(RTMP, Stream, #video_frame{content = audio, codec = aac, flavor = frame, dts = 23, pts = 23, body = <<"aac">>, sound = ?S}),
