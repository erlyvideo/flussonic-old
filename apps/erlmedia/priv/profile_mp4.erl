...
main([Path, Handler_|Args]) ->
  ...
  io:format("Keyframes length: ~240p~n", [length(video_frame:reduce_keyframes(Module:keyframes(R, [1,2])))]),

  N = case Args of
    [GopNum] -> list_to_integer(GopNum);
    [] -> length(video_frame:reduce_keyframes(Module:keyframes(R, [1,2]))) * 5 div 6
  end,
  {T3, {ok, Gop}} = timer:tc(fun() -> Module:read_gop(R, N, [1,2]) end),
