...
encode(pmt, Options) ->
  ...
  StreamInfos = proplists:get_value(streams, Options),
  
  PcrPid = case proplists:get_value(pcr_pid, Options) of
    undefined ->
      [{_, FirstPid, _}|_] = StreamInfos,
      FirstPid;
    Else -> Else
  end,  
  ...
  Streams = lists:map(fun({Codec, Pid, Info}) ->
    LangInfo = case proplists:get_value(lang, Info) of
      ...
      Lang_ when is_atom(Lang_) andalso Lang_ =/= undefined ->
        Lang = atom_to_binary(Lang_, latin1),
        <<16#0A, (size(Lang) + 1), Lang/binary, 0>>;
      Lang_ when is_list(Lang_) ->
        Lang = list_to_binary(Lang_),
        <<16#0A, (size(Lang) + 1), Lang/binary, 0>>;
      _ ->
        <<>>
    end,
    % ESInfo = <<LangInfo/binary, 16#1F, 2, Pid:16>>,
    ESInfo = <<LangInfo/binary>>,
...
extract_pmt(<<StreamType, 2#111:3, Pid:13, _:4, ESLength:12, ES:ESLength/binary, Rest/binary>>, Descriptors) ->
  Info = parse_es(ES),
  Entry = #pmt_entry{pid = Pid, codec = stream_codec(StreamType), info = Info},
  extract_pmt(Rest, [Entry|Descriptors]).


parse_es(ES) -> parse_es(ES, []).

parse_es(<<16#0A, Len, Lang:Len/binary, ES/binary>>, Info) ->
  parse_es(ES, [{lang,clean_nil(Lang)}|Info]);

parse_es(<<16#59, Len, _Tag:Len/binary, ES/binary>>, Info) ->
  parse_es(ES, Info);

parse_es(<<16#90, Len, _Tag:Len/binary, ES/binary>>, Info) ->
  parse_es(ES, Info);

parse_es(<<_, Len, _Tag:Len/binary, ES/binary>>, Info) ->
  parse_es(ES, Info);
...
stream_codec(_Type) -> {unhandled, _Type}.
